...
use tantivy::{
    doc, DocAddress, Index, IndexReader, IndexWriter, Score, Searcher, TantivyDocument, TantivyError,
    Term,
};
...
fn build_analyzers(locale: &str) -> Analyzers {
...
mod tests {
    ...
    fn matches_multi_segment_address() {
        ...
        assert_eq!(results[0].address_string, "Россия, Москва, улица Ленина, 12");
    }
    ...
    fn last_token_matches_as_prefix() {
        ...
        assert!(
            ...
            results.iter().map(|r| &r.address_string).collect::<Vec<_>>()
        );
    }
    ...
    fn kind_filter_selects_roads_and_buildings() {
        ...
        let buildings = fg
            ...
            .unwrap();
        assert!(buildings.iter().all(|r| r.geo_type == GeoObjectType::Building));
