...
func TestQuadtreeInsert(t *testing.T) {
	...
	var randomObject Bounds[string]
	numObjects := 1000
...
func TestQuadtreeRandomPointRetrieval(t *testing.T) {
	...
	if failure == false {
		t.Logf("Success: All the points were retrieved correctly, iterations: %d, objects: %d", iterations, numObjects)
	}
...
func TestIntersectionRetrieval(t *testing.T) {
	...
	inter := qt.RetrieveIntersections(Bounds[string]{
		X:      5,
		Y:      5,
		Width:  2.5,
		Height: 2.5,
...
func setupQuadtree(x float64, y float64, width float64, height float64) *Quadtree[string] {
